...
class BankControllerTest {
    ...
    @Transactional
    public void select() {
        ResponseEntity<?> response = bankController.select(authentication);
        assertNotNull(response.getBody());
        List<BankDto> list = (List<BankDto>) response.getBody();
        assertNotNull(list);
    }

    @Test
    void get() {
        ResponseEntity<?> response = bankController.get(UUID.randomUUID(), authentication);
    ...
    @Test
    void create() {
        ResponseEntity<?> response = bankController.create(null, authentication);
        ApiResponse api = (ApiResponse) response.getBody();
        assertNotNull(api);
    }
    ...
    @Test
    void update() {
        ResponseEntity<?> response = bankController.update(UUID.randomUUID(), null, authentication);
        ApiResponse api = (ApiResponse) response.getBody();
        assertNotNull(api);
    ...
    @Test
    void delete() {
        ResponseEntity<?> response = bankController.delete(UUID.randomUUID(), authentication);
        ApiResponse api = (ApiResponse) response.getBody();
        assertNull(api);
    }
}